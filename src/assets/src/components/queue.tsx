  ...
  return (
    <>
      <Row>
        <Col lg>
          <p className="mb-0">
            Select Meeting Type <span className="required">*</span>
          </p>
        </Col>
      </Row>
      <Row>
...
function QueueAttendingNotJoined(props: QueueAttendingProps) {
  const joinedOther =
    props.joinedQueue && props.joinedQueue.id !== props.queue.id;
  ...
  const notJoinedInpersonMeetingText =
    props.queue.inperson_location === "" ? (
      ...
      <>
        In-person meetings will take place at:{" "}
        <strong>{props.queue.inperson_location}</strong>
      </>
    );

  const controls =
    props.queue.status !== "closed" &&
    (joinedOther && props.joinedQueue ? (
      <>
        ...
        <JoinQueue
          queue={props.queue}
          backends={props.backends}
          onJoinQueue={props.onLeaveAndJoinQueue}
          disabled={props.disabled}
          selectedBackend={props.selectedBackend}
          onChangeSelectedBackend={props.onChangeBackend}
        />
      </>
    ) : (
      <JoinQueue
  ...
  const closedAlert = props.queue.status === "closed" && (
    <Alert variant="dark">
      <strong>This queue is currently closed.</strong> Please return at a later
      time or message the queue host to find out when the queue will be open.
    </Alert>
  );
  return (
    <>
        ...
        <Col>
          <ul>
            <li>
              ...
              <strong>{props.queue.line_length}</strong>
            </li>
...
const WaitingTurnAlert = (props: WaitingTurnAlertProps) => {
  const inPersonEnding =
    "-- we will tell you when the host is ready. Make sure you are nearby the in-person meeting location.";
  const videoMessageEnding = "so you can join the meeting once it is created.";

  const typeEnding =
    props.meetingType === "inperson" ? inPersonEnding : videoMessageEnding;

  const placeBeginning =
    props.placeInLine > 0
...
interface JoinedClosedAlertProps {
  meetingStatus: MeetingStatus;
}

const JoinedClosedAlert = (props: JoinedClosedAlertProps) => {
  const statusClause =
    props.meetingStatus === MeetingStatus.STARTED
      ? "your meeting is still in progress"
      : "you are still in line";
  return (
    <Alert variant="dark">
      This queue has been closed by the host, but {statusClause}. If you are
      unsure if your meeting will still happen, please contact the host.
    </Alert>
  );
};
...
const VideoMeetingInfo: React.FC<VideoMeetingInfoProps> = (props) => {
  const docLinkTag = (
    <a
      href={
        props.backend.docs_url === null ? undefined : props.backend.docs_url
      }
      target="_blank"
      className="card-link"
    >
        ...
        <Col md={6} sm={true}>
          <Card>
            <Card.Body>
              ...
              <Card.Text>
                Once the meeting is created, click Join Meeting to join the
                meeting and wait for the host. Download the app and test your
                audio now. Refer to {docLinkTag} for additional help getting
                started.
              </Card.Text>
            </Card.Body>
          </Card>
...
function QueueAttendingJoined(props: QueueAttendingProps) {
  ...
  // Alerts and head
  const closedAlert = props.queue.status === "closed" && (
    <JoinedClosedAlert meetingStatus={meeting.status} />
  );

  const turnAlert =
    meeting.line_place !== null ? (
      <WaitingTurnAlert
        meetingType={meetingBackend.name}
        placeInLine={meeting.line_place}
      />
    ) : (
      <MeetingReadyAlert meetingType={meetingBackend.name} />
    );

  const headText = inProgress
  ...
  const changeMeetingType = props.queue.my_meeting?.assignee ? (
    <span className="ms-2 fs-6">
      (A Host has been assigned to this meeting. Meeting Type can no longer be
      changed.)
    </span>
  ) : (
    <Button
    ...
    <div className="d-flex align-items-start mb-0 p-3">
      ...
      <div className="flex-grow-1">
        ...
        <p className="mb-0 text-dark">
          To receive an SMS (text) message when it's your turn:
          <br />
          add your cell phone number and enable attendee notifications in your{" "}
  ...
  const agendaBlock = !inProgress ? (
    <>
      <Card.Text>
        <strong>Meeting Agenda</strong> (Optional):
      </Card.Text>
  ...
  const joinLink =
    ...
    (meeting.backend_metadata!.meeting_url ? (
      ...
      <span>
        <strong>Please wait. A Join Meeting button will appear here.</strong>
      </span>
    ));

  const meetingInfo = isVideoMeeting && (
    <VideoMeetingInfo
      metadata={meeting.backend_metadata!}
      backend={meetingBackend}
    />
  );
...
function QueueAttending(props: QueueAttendingProps) {
  ...
  const yourQueueAlert = props.queue.hosts.find(
    (h) => h.username === props.user.username
  ) && (
    <>
      <br />
      <Alert variant="info">
        This is your queue, you can{" "}
        <Link to={"/manage/" + props.queue.id}>manage it</Link>.
      </Alert>
    </>
  );
  const footer = (
...
export function QueuePage(props: PageProps) {
  ...
  const userWebSocketError = useUserWebSocket(props.user!.id, (u) =>
    setMyUser(u as MyUser)
  );
  ...
  const [doLeaveQueue, leaveQueueLoading, leaveQueueError] =
    usePromise(leaveQueue);
  ...
  const [
    doLeaveAndJoinQueue,
    leaveAndJoinQueueLoading,
    leaveAndJoinQueueError,
  ] = usePromise(leaveAndJoinQueue);
  const changeAgenda = async (agenda: string) => {
    return await api.changeAgenda(queue!.my_meeting!.id, agenda);
  };
  const [doChangeAgenda, changeAgendaLoading, changeAgendaError] =
    usePromise(changeAgenda);
  const changeBackendType = async () => {
    const meeting = await api.changeMeetingType(
      queue!.my_meeting!.id,
      selectedBackend!
    );
  ...
  const [
    doChangeBackendType,
    changeBackendTypeLoading,
    changeBackendTypeError,
  ] = usePromise(changeBackendType);
  ...
  const queueDisplay = queue && selectedBackend && (
    <QueueAttending
      ...
      onLeaveQueue={(myMeeting: Meeting) =>
        confirmLeaveQueue(queue.status, myMeeting.status)
      }
  ...
  const meetingTypeDialog = queue && selectedBackend && (
    <ChangeMeetingTypeDialog
      queue={queue}
      backends={props.backends}
      show={showMeetingTypeDialog}
      onClose={() => setShowMeetingTypeDialog(false)}
      onSubmit={doChangeBackendType}
      selectedBackend={selectedBackend}
      onChangeBackend={setSelectedBackend}
    />
  );
