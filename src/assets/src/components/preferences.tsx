...
interface PreferencesEditorProps {
  ...
  onUpdateInfo: (
    phoneNumber: string,
    notifyMeAttendee: boolean,
    notifyMeHost: boolean,
    notifyMeAnnouncement: boolean
  ) => void;
...
function PreferencesEditor(props: PreferencesEditorProps) {
  ...
  const [countryDialCode, setCountryDialCode] = useState(
    props.user.phone_number !== ""
      ? props.user.phone_number.substring(0, 1)
      : ""
  );
  const [notifyMeAttendee, setNotifyMeAttendee] = useState(
    props.user.notify_me_attendee
  );
  const [notifyMeHost, setNotifyMeHost] = useState(props.user.notify_me_host);
  const [notifyMeAnnouncement, setNotifyMeAnnouncement] = useState(
    props.user.notify_me_announcement
  );
  ...
  const notifyMeAnnouncementInput = (
    <Form.Check
      type="checkbox"
      id="notify-me-announcement"
      className="mt-2"
      disabled={props.disabled}
      checked={notifyMeAnnouncement}
      onChange={() => setNotifyMeAnnouncement(!notifyMeAnnouncement)}
      label="As an attendee, I want to be notified via SMS when there is an announcement posted from my host. Announcements from other hosts are visible but not sent via SMS."
    />
  );
  ...
  const validateAndSubmit = (e: React.SyntheticEvent) => {
    ...
    const optInValidationErrors = [
      notifyMeAttendee &&
        !phoneNumberToSubmit &&
        new Error(
          "You must enter a phone number to opt in to attendee SMS notifications."
        ),
      notifyMeHost &&
        !phoneNumberToSubmit &&
        new Error(
          "You must enter a phone number to opt in to host SMS notifications."
        ),
      notifyMeAnnouncement &&
        !phoneNumberToSubmit &&
        new Error(
          "You must enter a phone number to opt in to announcement SMS notifications."
        ),
    ].filter((e) => e) as Error[];
    const otpValidationErrors = [
      changedPhoneNumber &&
        phoneNumberToSubmit !== "" &&
        new Error(
          "Please validate your new phone number before saving your preferences."
        ),
    ].filter((e) => e) as Error[];
    const validationErrors = [
      ...phoneValidationErrors,
      ...optInValidationErrors,
      ...otpValidationErrors,
    ];
  ...
  const alertBlock = (() => {
    switch (formStatus) {
      ...
      case FormStatus.SubmissionError:
        return (
          <Alert variant="danger">
            An error occurred while trying to update your preferences; please
            try again later.
          </Alert>
        );
    }
...
export function PreferencesPage(props: PageProps) {
  ...
  const [doRefresh, refreshLoading, refreshError] = usePromise(
    () => api.getUser(userId) as Promise<MyUser>,
    setUser
  );
  useEffect(() => {
    doRefresh();
  ...
  const [doUpdateInfo, updateInfoLoading, updateInfoError] = usePromise(
    (phoneNumber, notifyMeAttendee, notifyMeHost, notifyMeAnnouncement) =>
      api.updateUser(
        userId,
        phoneNumber,
        notifyMeAttendee,
  ...
  const doGetOneTimePassword = async (phoneNumberToSubmit: string) =>
    api.getOneTimePassword(userId, phoneNumberToSubmit) as Promise<unknown>;
  const doVerifyOneTimePassword = async (otp: string) => {
    const resp = (await api.verifyOneTimePassword(
      userId,
      otp
    )) as Promise<unknown>;
    await doRefresh();
