...
import {
  checkIfSetsAreDifferent,
  recordQueueManagementEvent,
  redirectToLogin,
} from "../utils";
import {
  confirmUserExists,
        ...
        <Col md={6} sm={9}>
          ...
          <Tab.Content aria-live="polite">
            ...
            <Tab.Pane eventKey="delete">
              <h2>Delete Queue</h2>
              <p>
                Delete the entire queue, including all hosts and current
                meetings in queue. <strong>This cannot be undone.</strong>
              </p>
              <div className="mt-4">
                <Button
                  variant="danger"
...
export function ManageQueueSettingsPage(props: PageProps) {
  ...
  const [showCorrectGeneralMessage, setShowCorrectGeneralMessage] =
    useState(false);
  const [showSuccessMessage, setShowSuccessMessage] = useState(false);

  const [name, setName] = useState("");
  const [nameValidationResult, validateAndSetNameResult, clearNameResult] =
    useStringValidation(queueNameSchema, true);
  const [description, setDescription] = useState("");
  const [
    descriptValidationResult,
    validateAndSetDescriptResult,
    clearDescriptResult,
  ] = useStringValidation(queueDescriptSchema, true);
  const [allowedMeetingTypes, setAllowedMeetingTypes] = useState(
    new Set() as Set<string>
  );
  const [
    allowedValidationResult,
    validateAndSetAllowedResult,
    clearAllowedResult,
  ] = useMeetingTypesValidation(props.backends, queue);
  const [inpersonLocation, setInpersonLocation] = useState("");
  const [
    locationValidationResult,
    validateAndSetLocationResult,
    clearLocationResult,
  ] = useStringValidation(queueLocationSchema, true);
  ...
  const setQueueChecked = (q: QueueAttendee | QueueHost | undefined) => {
    if (!q) {
      ...
      setAuthError(
        new Error(
          "You are not a host of this queue. If you believe you are seeing this message in error, contact the queue host(s)."
        )
      );
    }
  };
  ...
  const [doUpdateQueue, updateQueueLoading, updateQueueError] = usePromise(
    updateQueue,
    setQueueChecked
  );
  ...
  const [doRemoveHost, removeHostLoading, removeHostError] =
    usePromise(removeHost);
  const confirmRemoveHost = (h: User) => {
    setStateAndOpenDialog(
      "Remove Host?",
      `Are you sure you want to remove host ${h.username}?`,
      () => doRemoveHost(h)
    );
  };
  ...
  const [doRemoveQueue, removeQueueLoading, removeQueueError] =
    usePromise(removeQueue);
  ...
  const handleSaveGeneralClick = () => {
    ...
    if (
      ...
      const descriptForUpdate =
        description.trim() !== queue?.description ? description : undefined;
      const allowedForUpdate = checkIfSetsAreDifferent(
        new Set(queue!.allowed_backends),
        allowedMeetingTypes
      )
        ? allowedMeetingTypes
        : undefined;
      const locationForUpdate =
        inpersonLocation.trim() !== queue?.inperson_location
          ? inpersonLocation
          : undefined;
      if (
        nameForUpdate !== undefined ||
        descriptForUpdate !== undefined ||
        allowedForUpdate ||
  ...
  const queueSettingsEditor = queue && (
    <QueueSettingsEditor
      ...
      checkHostError={
        addHostError ? { source: "Add Host", error: addHostError } : undefined
      }
  ...
  return (
    <div>
      <Helmet>
        <title>{getPageTitle("Manage Queue Settings")}</title>
      </Helmet>
      <Dialog {...dialogState} />
      ...
      <Breadcrumbs
        ...
        intermediatePages={[
          ...
          {
            title: queue?.name ?? queueIDInt.toString(),
            href: `/manage/${queueIDInt}`,
          },
        ]}
      />
